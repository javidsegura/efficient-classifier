...
from library.phases.dataset.split.strategies.noTimeSeries import NoTimeSeries
from library.phases.dataset.split.strategies.timeSeries import TimeSeries
...
class Dataset:
    ...
    def __init__(self, dataset_path: str, model_type: str, random_state: int = RANDOM_STATE) -> None:
      """
      ...
      model_type : str
          The type of model to use (e.g: classification_timeSeries)
      random_state : int
          The random state to use
      """ 
      assert model_type in ["classification_timeSeries", "regression_timeSeries", "classification", "regression"], "The task must be either classification or regression"
      self.df = pd.read_csv(dataset_path)
      self.random_state = random_state

      splitted_type = model_type.split("_")
